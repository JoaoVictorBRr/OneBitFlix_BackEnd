...
function filterLastEpisodesByCourse(episodes: EpisodeInstance[]) {
    ...
    const lastEpisodes = episodes.reduce((currentList, episode) => {
        ...
        const lisWithoutEpisodeFromSameCourse = currentList.filter(ep => ep.courseId !== episode.courseId)
        lisWithoutEpisodeFromSameCourse.push(episode)

        return lisWithoutEpisodeFromSameCourse
    }, [] as EpisodeInstance[])
    return lastEpisodes
}
...
export const userService = {
    ...
    update: async (id: number, attributes: {
        ...
        const [affectedRowns, updatedUsers] = await User.update(attributes, { where: { id }, returning: true })

        return updatedUsers[0]
    },

    updatePassword: async (id: number, password: string) => {
        const [affectedRowns, updatedUsers] = await User.update({password}, 
            {where: { id },
            returning: true,
